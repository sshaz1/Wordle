...
/**
 * Controller class for the Wordle game interface (medium mode).
 *
 * @author Emily Christina Atyeo (251281573)
 */
public class WordleGameplayControllerMedium implements WordleGameplayController{
    ...
    private boolean gameWon = false; // Initialize to false
    ...
    private static int playerScore = 600;
    ...
    public void handleCheckGuess() {
        ...
        if (guess.length() != 4) {
            feedbackText.setText("Invalid guess. Please enter a 4-letter word.");
        ...
        if (numberOfGuesses == 1) {
            Label[] row1 = {box00, box01, box02, box03};
            checkGuess(row1);
            playerScore = playerScore - 100;

        } else if (numberOfGuesses == 2) {
            Label[] row2 = {box05, box06, box07, box08};
            checkGuess(row2);
            playerScore = playerScore - 100;

        } else if (numberOfGuesses == 3) {
            Label[] row3 = {box10, box11, box12, box13};
            checkGuess(row3);
            playerScore = playerScore - 100;

        } else if (numberOfGuesses == 4) {
            Label[] row4 = {box15, box16, box17, box18};
            checkGuess(row4);
            playerScore = playerScore - 100;

        } else if (numberOfGuesses == 5) {
            Label[] row5 = {box20, box21, box22, box23};
            checkGuess(row5);
            playerScore = playerScore - 100;

        } else if (numberOfGuesses == 6) {
            Label[] row6 = {box25, box26, box27, box28};
            checkGuess(row6);
            playerScore = playerScore - 100;
        }
    }
    ...
    private void onContinueButtonClicked(MouseEvent event) throws IOException {
        ...
        resultController.setPlayerScore(playerScore+100);
